...
class DeterministicCalendarProvider:
    ...
    def conflict_truth(self, candidate: CandidateCalendarAction) -> list[dict[str, Any]]:
        ...
        events = [RawCalendarEvent.from_dict(row["event"]) for row in self.state.get("events", {}).values()]
        for action in self._expanded_actions(candidate.actions):
    ...
    def verify(self, transaction: Any, *, observation: RawCalendarObservation | None = None) -> ProviderVerificationResult:
        ...
        existing = set(self.state.get("events", {}).keys())
        verified = [x for x in external_ids if x in existing or x in payload.get("deleted_external_ids", [])]
        missing = [x for x in external_ids if x not in verified]
        local_time_echo_ok = True
        for external_id in payload.get("created_external_ids", []) + payload.get("moved_external_ids", []):
            if external_id not in self.state.get("events", {}):
                local_time_echo_ok = False
        rollback_handle = payload.get("rollback_handle_id")
        rollback_record = self.state.get("rollback_records", {}).get(rollback_handle or "", {})
        return ProviderVerificationResult(
            provider_id=self.provider_id,
            status="verified" if not missing and local_time_echo_ok else "unverified",
            verified_external_ids=verified,
            missing_external_ids=missing,
            rollback_handle_id=rollback_handle,
            rollback_verified=bool(rollback_record.get("rollback_verified", False)) if rollback_handle else None,
    ...
    def _expanded_actions(self, actions: list[AtomicCalendarAction]) -> list[AtomicCalendarAction]:
    ...
    def _event_from_action(external_id: str, action: AtomicCalendarAction) -> RawCalendarEvent:
        ...
        category = {
            ...
            AtomicActionType.AUTO_APPLY_PLAN: "auto_plan",
