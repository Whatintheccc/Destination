...
def authority_scopes_for_tier(max_authority_tier: int) -> list[str]:
    """Default scoped authority lattice shared by planner, self-play, and app demos."""
    tier = max(0, min(6, int(max_authority_tier)))
    scopes = ["recommend", "stage", "commit_private", "undo"]
    if tier >= 5:
        scopes.extend(["commit_social", "move_people_meeting", "send_calendar_update"])
    if tier >= 6:
        scopes.append("auto_apply_plan")
    return scopes
...
class SemanticSignal:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "SemanticSignal":
        return cls(
            ...
            created_by=str(data.get("created_by", "estimator")),
            evidence=[str(x) for x in data.get("evidence", [])],
            evidence_window=dict(data.get("evidence_window", {})),
            confidence=float(data.get("confidence", 0.0)),
            half_life_days=int(data.get("half_life_days", 28)),
...
class CandidateCalendarAction:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "CandidateCalendarAction":
        try:
            ...
            normalized_intent = normalize_intent(data.get("intent", "unknown"))
        except Exception:
            normalized_intent = {"intent": str(data.get("intent", "unknown")), "intent_raw": str(data.get("intent", "unknown")), "matched_by": "unavailable"}
        return cls(
            candidate_id=data["candidate_id"],
            intent=data.get("intent_canonical", data.get("intent", normalized_intent["intent"])),
            intent_raw=data.get("intent_raw", normalized_intent.get("intent_raw", data.get("intent", "unknown"))),
            intent_matched_by=data.get("intent_matched_by", normalized_intent.get("matched_by", "unknown")),
...
class PolicyTuning:
    ...
    source_report: str = ""
    bias_evidence: dict[str, dict[str, Any]] = field(default_factory=dict)
    frontier_effect: dict[str, Any] = field(default_factory=dict)
    ...
    def from_dict(cls, data: dict[str, Any]) -> "PolicyTuning":
        return cls(
            ...
            bias_evidence={str(k): dict(v) for k, v in data.get("bias_evidence", {}).items() if isinstance(v, dict)},
            frontier_effect=dict(data.get("frontier_effect", {})),
        )
