...
class BiographyStore:
    ...
    def update_from_reward(
        ...
        *,
        ...
        # P12: reward events no longer mutate unobservable biography scalars.
        # Observable dismiss/ignore/useful actions stay in the ActionStream;
        # versioned estimators derive semantic signals from those rows.
        behavioral_prior = biography.confidence_for("action_stream_feedback")
        if reward.notification_dismissed or reward.ignored:
            reason = "action-stream dismissal/ignore observed; estimator owns derived interruption state"
            next_value = max(0.0, behavioral_prior - 0.01)
        elif reward.accepted or reward.explicit_useful:
            reason = "action-stream acceptance/useful observed; estimator owns derived interruption state"
            next_value = min(1.0, behavioral_prior + 0.01)
        else:
            reason = "neutral action-stream reward observed"
            next_value = behavioral_prior
        updated = replace(biography, last_profile_update_at=observed_at)
        event = self._event(
            biography=biography,
            claim="action_stream_feedback",
            prior=behavioral_prior,
