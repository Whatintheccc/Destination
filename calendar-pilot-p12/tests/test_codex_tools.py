...
from calendar_pilot.diffusiongemma.live import LiveDiffusionGemmaCredentialError, LiveDiffusionGemmaSchemaError
...
class CodexToolRuntimeTests(unittest.TestCase):
    ...
    def test_live_policy_schema_failure_reports_model_retry_recovery(self):
        runtime = CodexToolRuntime(policy=SchemaFailingLivePolicy())
        receipt = runtime.execute(
            CodexToolCall("frontier_bad_schema", CodexToolName.GENERATE_CANDIDATE_FRONTIER, {"limit": 4}, 3, "frontier"),
            load_obs(),
            load_bio(),
        )

        self.assertEqual(receipt.status, CodexToolStatus.FAILED)
        self.assertEqual(receipt.output["error_category"], "model_policy_schema_failure")
        self.assertIn("NIM was reached", receipt.output["recovery"])
        self.assertNotIn("Configure NVIDIA NIM credentials", receipt.output["recovery"])
...
class SchemaFailingLivePolicy:
    backend_name = "nvidia_nim_diffusiongemma_policy"

    def generate_candidates(self, *_args, **_kwargs):
        raise LiveDiffusionGemmaSchemaError("NIM frontier JSON response was invalid after 2 attempts")
