...
final class CalendarPilotKernelTests: XCTestCase {
    ...
    func testSocialActuationRequiresCommitSocialScopeThenMaterializes() throws {
        let eventStart = date("2026-07-01T12:00:00-07:00")
        let eventEnd = date("2026-07-01T12:30:00-07:00")
        let observation = RawCalendarObservation(
            observationID: "obs_social",
            userScopeID: "u",
            observedAt: date("2026-07-01T08:00:00-07:00"),
            timeZoneID: "America/Los_Angeles",
            events: [RawCalendarEvent(eventID: "evt_social", title: "People meeting", start: eventStart, end: eventEnd, calendarID: "work", attendees: ["other@example.com"], isUserOwned: false)]
        )
        let candidate = CandidateCalendarAction(
        ...
        let deniedKernel = CalendarKernel()
        let deniedGrant = grant(deniedKernel, tier: 5, observation: observation, scopes: ["recommend", "stage", "commit_private", "undo"])
        let (denied, _) = deniedKernel.authorizeAndMaterialize(candidate: candidate, observation: observation, authorityGrant: deniedGrant, requestedAuthorityTier: 5)
        XCTAssertEqual(denied.syncStatus, .denied)
        XCTAssertTrue(denied.deniedReason?.contains("commit_social") == true)

        let kernel = CalendarKernel()
        let g = grant(kernel, tier: 5, observation: observation, scopes: ["recommend", "stage", "commit_social", "undo"])
        let (receipt, events) = kernel.authorizeAndMaterialize(candidate: candidate, observation: observation, authorityGrant: g, requestedAuthorityTier: 5)
        XCTAssertEqual(receipt.syncStatus, .materialized)
        XCTAssertEqual(events.first?.start, date("2026-07-01T13:00:00-07:00"))
    }
    ...
    func testTierSixAutoApplyPlanMaterializesNestedActions() throws {
        let observation = emptyObservation()
        let nested = """
        [{"action_type":"create_focus_block","title":"Protected work","start":"2026-07-01T09:00:00-07:00","end":"2026-07-01T10:00:00-07:00","calendar_id":"work"}]
        """
        let candidate = CandidateCalendarAction(
            candidateID: "cand_auto_plan",
            intent: "auto_apply_plan",
            actions: [AtomicCalendarAction(actionType: .autoApplyPlan, title: "Apply optimizer plan", calendarID: "work", metadata: ["plan_actions": nested])],
            targetCalendars: ["work"],
            affectedEventIDs: [],
            affectedPeopleIDs: [],
            reversibility: .high,
            requiredAuthorityTier: 6
        )
        let kernel = CalendarKernel()
        let g = grant(kernel, tier: 6, observation: observation, scopes: ["recommend", "stage", "commit_private", "auto_apply_plan", "undo"])
        let (receipt, events) = kernel.authorizeAndMaterialize(candidate: candidate, observation: observation, authorityGrant: g, requestedAuthorityTier: 6)
        XCTAssertEqual(receipt.syncStatus, .materialized)
        XCTAssertEqual(events.count, 1)
        XCTAssertEqual(events.first?.category, "focus")
    }
