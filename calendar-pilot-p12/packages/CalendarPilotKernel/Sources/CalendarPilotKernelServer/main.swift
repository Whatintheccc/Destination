...
struct ContractRoundtripPayload: Codable {
    var candidate: CandidateCalendarAction?
    var observation: RawCalendarObservation?
    var receipt: CalendarActionReceipt?
    var authorityGrant: AuthorityGrant?
...
final class KernelRPCServer {
    ...
    private func handle(_ request: KernelRPCRequest) throws -> KernelRPCResponse {
        ...
        case "undo":
            ...
            return try ok(request, ["receipt": toJSONValue(receipt)])
        case "contract_roundtrip":
            let payload = try decode(ContractRoundtripPayload.self, from: request.payload)
            var out: [String: JSONValue] = [:]
            if let candidate = payload.candidate { out["candidate"] = try toJSONValue(candidate) }
            if let observation = payload.observation { out["observation"] = try toJSONValue(observation) }
            if let receipt = payload.receipt { out["receipt"] = try toJSONValue(receipt) }
            if let grant = payload.authorityGrant { out["authority_grant"] = try toJSONValue(grant) }
            return try ok(request, out)
        default:
