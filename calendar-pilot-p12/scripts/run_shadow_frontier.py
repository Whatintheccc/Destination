...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--observation',required=True); ap.add_argument('--profile',default='data/sample_profile.json'); ap.add_argument('--family',default=''); ap.add_argument('--out',default='runs/p12_shadow_frontier.json'); args=ap.parse_args()
    ...
    if family:
        candidates=[c for c in candidates if c.get('intent') == family]
    payload={'shadow_frontier_schema_version':'shadow_frontier.v1','observation_id':obs.observation_id,'mode':'shadow_no_commit','family':family,'candidate_count':len(candidates),'candidates':candidates,'signal_stream':'system'}
