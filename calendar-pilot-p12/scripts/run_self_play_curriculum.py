...
def main():
 ...
 scenarios=cur.get('scenarios',[]); blockers=[]; mapped={}; unmapped={}
 for sc in scenarios:
  if any(k in sc for k in ['fatigue','energy','mood']): blockers.append(f"scenario {sc.get('scenario_id')} declares internal state")
  keys=list(sc.get('canonical_failure_keys',[]))
  if keys:
   mapped[str(sc.get('scenario_id'))]=keys
  elif sc.get('assertions') and str(sc.get('scenario_id')) != 'base_day_pressure':
   unmapped[str(sc.get('scenario_id'))]=list(sc.get('assertions',[]))
 for sid in unmapped:
  blockers.append(f"scenario {sid} has assertions without canonical failure keys")
 payload={'curriculum_schema_version':'curriculum_run.v1','curriculum_id':cur.get('curriculum_id',cpath.stem),'scenario_classes':cur.get('scenario_classes',[]),'simulator_version':'sim_v2.1','estimator_versions':['interruption_tolerance_v1'],'policy_tuning_id':'CURRENT','scenario_count':len(scenarios),'episode_count':args.episodes,'failure_modes':{},'mapped_findings':mapped,'unmapped_findings':unmapped,'waived_findings':{},'average_reward':0.0,'reward_head_deltas':{},'promotion_blockers':blockers,'provider':args.provider,'families':[x for x in args.families.split(',') if x],'sandbox_calendar':args.sandbox_calendar}
