...
from calendar_pilot.types import RawCalendarObservation
...
def main():
    ...
    payload={'ok':True,'signals':[signal],'report':report}
    dest=Path(args.out); dest=dest if dest.is_absolute() else ROOT/dest
    atomic_write_json(dest,payload)
    if args.replay_out:
        replay=ReplayBuffer()
        replay.append_semantic_signal(signal, trace_id=signal['signal_id'])
        replay.append_signal_estimator_report(report, trace_id=report['report_id'], causal_parent_id=f"semantic_signal:{signal['signal_id']}")
