...
**Status input (from the completed `P12-test.md` run `20260703T224814Z-p12`, updated by Stage D2 and Step E evidence):** overall P12 baseline decision **pass**, and Step E is now **complete/pass** at run `20260706T220150Z-step-e-complete`. The release/testing instrument is pinned by that run bundle's `INSTRUMENT.sha`; `make p12-release` reports `decision: pass`; `reward_heads`, `policy_ablation`, calibration, Belief/object-level `explain()`, and explicit `CalendarPilot SelfPlay` EventKit mutation/rollback are all covered by evidence. The wave harness now exists: every behavior-changing wave must carry an eight-field experiment record, C-VAR seed-bootstrap variance/borderline flip-rate report, B_migrate dual-run assertion report, and release evidence before it can claim a promotion or safe migration. Two data-quality flags recorded in the fixture measurement snapshot must still be carried forward, not forgotten: `OTHER_intent_rate: 0.1429` (above the 0.10 gate at fixture scale) and `expected_intent_hit_rate: 0.0`.
...
| Step E instrument gate | Codex | verified | `calendar-pilot-p12/runs/p12_next_evidence/20260706T220150Z-step-e-complete/instrument/` | `make p12-release` pass plus protected Step E legs run or signed-root-listed | Step E complete; do not start destructive waves until the separate lineage/root-set prerequisites are ready |
| C-VAR variance gate | Codex | verified | `calendar-pilot-p12/scripts/run_cvar_report.py`, `calendar-pilot-p12/experiments/configs/cvar_seed_set.json`, `calendar-pilot-p12/experiments/promoted/CURRENT.json`, and `calendar-pilot-p12/runs/p12_release/cvar_report.json` | C-VAR report pass for before/current vs after candidate on every behavior-changing wave | require C-VAR in each eight-field experiment record before P13 organ migration or compression verdicts |
...
| B-005 | instrument | Codex | **resolved** | C-VAR pre-wave certificate | Resolved: Step E now provides a C-VAR report over a frozen seed set, pinned `CURRENT.json`, seeded bootstrap variance, and borderline top-candidate flip-rate; `make p12-release` includes the C-VAR leg. | require a fresh C-VAR before/after report in every behavior-changing wave's experiment record |
...
Update — Step E wave-harness follow-up adds the missing promotion harness before any P13 organ migration: C-VAR now computes a report from a frozen seed set plus pinned `experiments/promoted/CURRENT.json`, seeded bootstrap variance, promotion-decision comparison, and borderline top-candidate flip-rate; B_migrate now has a dual-run assertion harness for old `DogfoodSessionState` snapshots versus projected `view_state.v2`; the eight-field experiment-record schema/template is versioned; and `interruption_tolerance_v1` now emits a real `Belief` object and replay row. `make p12-release` includes `cvar` and `b_migrate`; every behavior-changing wave must cite those reports in its experiment record. No frontend organ migration, deletion, or compression work landed in this follow-up.
