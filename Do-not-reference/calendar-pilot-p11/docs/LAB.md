...
The lab keeps a closed intent vocabulary for regression metrics. Draft-plan actions such as `draft_day_repair_plan` and `draft_schedule_plan` are normalized to `ask_clarification`, because they stage a user-reviewed plan rather than adding a new calendar-writing family. The raw intent remains in replay as `intent_raw` so taxonomy changes stay auditable.
...
The live replay/offline tuning loop is deliberately separated from deterministic promotion evidence. If live NIM returns invalid frontier JSON after schema retry, `make replay-offline-tuning-loop` records a replay-visible `model_generation_rejection` plus `nim_schema_failure.json` with `decision=controlled_hold` by default. Set `CALENDAR_PILOT_REQUIRE_LIVE_NIM=1` to fail closed when running a strict live-provider gate.
