...
class AppleEventKitProvider:
    ...
    def verify(self, transaction: Any, *, observation: RawCalendarObservation | None = None) -> ProviderVerificationResult:
        ...
        try:
            verification = self.bridge.call("verify_events", {"external_ids": external_ids})
            found = {str(external_id) for external_id in verification.get("verified_external_ids", [])}
            local_time_echo_ok = bool(verification.get("local_time_echo_ok", False))
        except Exception:
            try:
