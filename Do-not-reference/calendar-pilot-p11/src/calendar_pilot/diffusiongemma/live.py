...
from calendar_pilot.types import AtomicActionType, CandidateCalendarAction, PolicyTuning, RawCalendarObservation, UserBiography, parse_dt, to_jsonable
            ...
            rejection_summary = [
                {
                    ...
                    "schema_errors": rejection.get("schema_errors", [])[:3] if isinstance(rejection.get("schema_errors"), list) else [],
                }
                for rejection in rejections[:3]
            ]
            raise LiveDiffusionGemmaSchemaError(
                "NIM frontier response did not contain any valid typed candidates; "
                f"validation_errors={validation_errors[:5]}; rejections={rejection_summary}"
            )
        ...
        for action_idx, action in enumerate(normalized.get("actions", [])):
            ...
            if isinstance(params, dict):
                for param_key, param_value in params.items():
                    action_payload.setdefault(param_key, param_value)
                validation_errors.append(f"normalized_action_params:{idx}:{action_idx}")
            ...
            if "start" not in action_payload and "new_start" in action_payload:
                action_payload["start"] = action_payload["new_start"]
                validation_errors.append(f"normalized_action_new_start_alias:{idx}:{action_idx}")
            if "end" not in action_payload and "new_end" in action_payload:
                action_payload["end"] = action_payload["new_end"]
                validation_errors.append(f"normalized_action_new_end_alias:{idx}:{action_idx}")
            if str(action_payload.get("action_type", "")).lower() == "batch_tasks" and "start" not in action_payload and "target_time" in action_payload:
                start = parse_dt(action_payload.get("target_time"))
                if start is not None:
                    duration_minutes = int(action_payload.get("duration_minutes") or action_payload.get("duration") or 30)
                    action_payload["start"] = start.isoformat()
                    action_payload["end"] = (start + timedelta(minutes=max(1, duration_minutes))).isoformat()
                    validation_errors.append(f"normalized_batch_tasks_target_time:{idx}:{action_idx}")
            if "metadata" not in action_payload:
                metadata = {}
                for metadata_key in ("notes", "description", "reason", "target_time"):
                    if metadata_key in action_payload:
                        metadata[metadata_key] = str(action_payload[metadata_key])
                if "task_ids" in action_payload:
                    task_ids = action_payload["task_ids"]
                    if isinstance(task_ids, list):
                        metadata["task_ids"] = ",".join(str(task_id) for task_id in task_ids)
                    else:
                        metadata["task_ids"] = str(task_ids)
                if metadata:
                    action_payload["metadata"] = metadata
                    validation_errors.append(f"normalized_action_metadata_aliases:{idx}:{action_idx}")
...
class LiveDiffusionGemmaPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        self.temporal_controller = RightMomentTemporalController()
        ...
        *,
        ...
        if callable(generate_frontier):
            ...
            for idx, candidate in enumerate(result.candidates, start=1):
                ...
                self.temporal_controller.apply(candidate, observation, biography)
