...
private final class EventKitCommandHandler {
    ...
    func handle(command: String, payload: [String: JSONValue]) async throws -> [String: JSONValue] {
        ...
        case "verify_events":
            return try verifyEvents(payload)
        case "rollback":
    ...
    private func verifyEvents(_ payload: [String: JSONValue]) throws -> [String: JSONValue] {
        try requireAuthorized()
        let sandboxCalendarID = payload["sandbox_calendar_id"]?.stringValue ?? envSandboxCalendarID
        let externalIDs = stringArray(payload["external_ids"])
        var verified: [String] = []
        var missing: [String] = []
        for eventID in externalIDs {
            guard let event = store.event(withIdentifier: eventID) else {
                missing.append(eventID)
                continue
            }
            if let sandboxCalendarID, !sandboxCalendarID.isEmpty {
                let calendarID = event.calendar?.calendarIdentifier ?? ""
                let calendarTitle = event.calendar?.title ?? ""
                if calendarID != sandboxCalendarID && calendarTitle != sandboxCalendarID {
                    throw BridgeFailure.sandboxViolation("verify attempted to inspect non-sandbox event '\(eventID)'")
                }
            }
            verified.append(eventID)
        }
        return [
            "provider_id": JSONValue("apple_eventkit"),
            "verified_external_ids": JSONValue(verified.map(JSONValue.init)),
            "missing_external_ids": JSONValue(missing.map(JSONValue.init)),
            "local_time_echo_ok": JSONValue(missing.isEmpty),
        ]
    }
