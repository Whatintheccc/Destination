...
class KernelStubTests(unittest.TestCase):
    ...
    def test_receipt_id_distinguishes_authority_outcomes(self):
        self.candidate.reversibility = Reversibility.HIGH
        kernel = SwiftKernelStub()
        ...
        committed = kernel.authorize_and_materialize(
            self.candidate,
            self.observation,
            authority_grant=commit_grant.grant_id,
            requested_authority_tier=3,
            correlation_id="same_candidate_attempt",
        )
        denied = kernel.authorize_and_materialize(
            self.candidate,
            self.observation,
            authority_grant=denied_grant.grant_id,
