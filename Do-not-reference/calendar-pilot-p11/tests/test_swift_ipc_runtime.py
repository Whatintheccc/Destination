...
class SwiftIPCRuntimeTests(unittest.TestCase):
    ...
    def test_commit_receipt_id_distinguishes_authority_outcomes(self) -> None:
        observation = load_observation()
        candidate = focus_candidate("cand_ipc_receipt_outcome")
        with SwiftKernelIPCClient(package_path=ROOT / "packages" / "CalendarPilotKernel") as kernel:
            commit_grant = kernel.issue_authority_grant(
                user_scope_id=observation.user_scope_id,
                max_authority_tier=3,
                scopes=["recommend", "stage", "commit_private", "undo"],
                confirmation_provenance="ipc_receipt_commit",
                confirmed_by_user=True,
                issued_at=observation.observed_at,
            )
            denied_grant = kernel.issue_authority_grant(
                user_scope_id=observation.user_scope_id,
                max_authority_tier=0,
                scopes=["recommend"],
                confirmation_provenance="ipc_receipt_denied",
                confirmed_by_user=True,
                issued_at=observation.observed_at,
            )

            committed = kernel.authorize_and_materialize(candidate, observation, authority_grant=commit_grant.grant_id, requested_authority_tier=3, correlation_id="same_candidate_attempt")
            denied = kernel.authorize_and_materialize(candidate, observation, authority_grant=denied_grant.grant_id, requested_authority_tier=0, correlation_id="same_candidate_attempt")

            self.assertEqual(committed.sync_status, "materialized")
            self.assertEqual(denied.sync_status, "denied")
            self.assertNotEqual(committed.receipt_id, denied.receipt_id)
    ...
    def test_live_provider_restore_rehydrates_swift_undo_ledger(self) -> None:
        with tempfile.TemporaryDirectory() as td, patch.dict(os.environ, {
            ...
            session = DogfoodSessionState(run_dir=run_dir)
            observation = session.observation
            candidate = focus_candidate("cand_live_provider_restore")
            grant = session.issue_authority_grant(
                confirmed=True,
                scopes=["commit_private", "undo"],
                reason="live_provider_restore_test",
            )
            committed = session.kernel.authorize_and_materialize(
                candidate,
                observation,
                authority_grant=grant.grant_id,
                requested_authority_tier=3,
                correlation_id="trace_live_provider_restore_commit",
            )
            rollback = committed.rollback_handle_id or ""
            self.assertTrue(rollback)
            self.assertIn(rollback, session.kernel.undo_ledger)
            ...
            reloaded = DogfoodSessionState(run_dir=run_dir)
            self.assertEqual(reloaded.runtime_report()["runtime_mode"], "live_provider")
            self.assertEqual(reloaded.runtime_report()["backends"]["kernel"], "SwiftKernelIPCClient")
            self.assertIn(rollback, reloaded.kernel.undo_ledger)
