...
public struct CandidateCalendarAction: Codable, Hashable, Sendable {
    ...
    public var intentMatchedBy: String
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case intent
        case intentRaw = "intent_raw"
        case intentMatchedBy = "intent_matched_by"
        case actions
    ...
    public init(candidateID: String, intent: String, intentRaw: String = "", intentMatchedBy: String = "unknown", actions: [AtomicCalendarAction], targetCalendars: [String], affectedEventIDs: [String], affectedPeopleIDs: [String], reversibility: ReversibilityBand, requiredAuthorityTier: Int, predictedAcceptance: Double = 0, predictedUtility: Double = 0, predictedEngagement: Double = 0, predictedRegret: Double = 0, predictedInterruptionCost: Double = 0, predictedSocialRisk: Double = 0, predictedLongHorizonValue: Double = 0, expectedReward: Double = 0, recommendedExecutionTime: Date? = nil, rightMomentDecision: RightMomentDecision = .doNothing, explanation: String = "", modelStory: [String] = [], counterfactual: String = "", controlNotes: [String] = [], rewardBreakdown: [String: Double] = [:], rightMomentScore: Double = 0, simulatedOutcomes: [String: Double] = [:]) {
        self.candidateID = candidateID
        self.intent = intent
        self.intentRaw = intentRaw.isEmpty ? intent : intentRaw
        self.intentMatchedBy = intentMatchedBy
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.intentRaw = try c.decodeIfPresent(String.self, forKey: .intentRaw) ?? self.intent
        self.intentMatchedBy = try c.decodeIfPresent(String.self, forKey: .intentMatchedBy) ?? "unknown"
...
public struct RewardEvent: Codable, Hashable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        self.rewardEventID = try c.decode(String.self, forKey: .rewardEventID)
        self.receiptID = try c.decode(String.self, forKey: .receiptID)
        self.observedAt = try c.decode(Date.self, forKey: .observedAt)
        self.accepted = try c.decodeIfPresent(Bool.self, forKey: .accepted)
        self.edited = try c.decodeIfPresent(Bool.self, forKey: .edited)
        self.undone = try c.decodeIfPresent(Bool.self, forKey: .undone)
        self.deletedLater = try c.decodeIfPresent(Bool.self, forKey: .deletedLater)
        self.ignored = try c.decodeIfPresent(Bool.self, forKey: .ignored)
        self.explicitUseful = try c.decodeIfPresent(Bool.self, forKey: .explicitUseful)
        self.explicitWrong = try c.decodeIfPresent(Bool.self, forKey: .explicitWrong)
        self.explicitNotNeeded = try c.decodeIfPresent(Bool.self, forKey: .explicitNotNeeded)
        self.notificationDismissed = try c.decodeIfPresent(Bool.self, forKey: .notificationDismissed)
        self.survivedUntilEvent = try c.decodeIfPresent(Bool.self, forKey: .survivedUntilEvent)
        self.downstreamConflict = try c.decodeIfPresent(Bool.self, forKey: .downstreamConflict)
        self.reengaged = try c.decodeIfPresent(Bool.self, forKey: .reengaged)
        self.utilityReward = try c.decodeIfPresent(Double.self, forKey: .utilityReward) ?? 0
        self.acceptanceReward = try c.decodeIfPresent(Double.self, forKey: .acceptanceReward) ?? 0
        self.engagementReward = try c.decodeIfPresent(Double.self, forKey: .engagementReward) ?? 0
        self.regretPenalty = try c.decodeIfPresent(Double.self, forKey: .regretPenalty) ?? 0
        self.interruptionPenalty = try c.decodeIfPresent(Double.self, forKey: .interruptionPenalty) ?? 0
        self.socialRiskPenalty = try c.decodeIfPresent(Double.self, forKey: .socialRiskPenalty) ?? 0
        self.totalReward = try c.decodeIfPresent(Double.self, forKey: .totalReward) ?? 0
        self.provenance = try c.decodeIfPresent(String.self, forKey: .provenance) ?? "observed"
    }
}
