...
class DeterministicCalendarProvider:
    ...
    def _expanded_actions(self, actions: list[AtomicCalendarAction]) -> list[AtomicCalendarAction]:
        ...
        for action in actions:
            if action.action_type == AtomicActionType.AUTO_APPLY_PLAN and action.metadata.get("plan_actions"):
                try:
                    payload = json.loads(action.metadata["plan_actions"])
                    nested = [AtomicCalendarAction.from_dict(item) for item in payload if isinstance(item, dict)]
                except Exception:
                    nested = []
                if nested:
