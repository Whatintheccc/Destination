...
def _expanded_actions(candidate: CandidateCalendarAction) -> list[AtomicCalendarAction]:
    ...
    for action in candidate.actions:
        if action.action_type == AtomicActionType.AUTO_APPLY_PLAN:
            encoded = action.metadata.get("plan_actions", "")
            try:
                rows = json.loads(encoded) if encoded else []
            except json.JSONDecodeError:
                rows = []
            if isinstance(rows, list):
                nested: list[AtomicCalendarAction] = []
                for row in rows:
                    if isinstance(row, dict):
                        try:
                            nested.append(AtomicCalendarAction.from_dict(row))
