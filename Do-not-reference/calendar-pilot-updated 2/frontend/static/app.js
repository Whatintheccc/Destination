...
async function pollForAssistantTurn(startMessageCount, timeoutMs = 60000) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    const latest = (state.summary || {}).latest_turn || {};
    if (messages.length > startMessageCount && latest.role === 'assistant') return state;
  }
  return lastState || await loadState();
}
...
function renderChat(chat) {
  ...
  const inlineCandidateCards = messages.some(message => (message.cards || []).some(card => card.type === 'candidate'));
  const inlineReceiptCards = messages.some(message => (message.cards || []).some(card => card.type === 'receipt'));
  let html = messages.map(renderMessage).join('');
  if (!inlineCandidateCards && (chat.candidate_cards || []).length) {
    html += renderMessage({
      role: 'assistant',
      title: 'Candidate actions',
      body: 'These candidates are available from the latest plan.',
      cards: (chat.candidate_cards || []).slice(0, 3),
    });
  }
  if (!inlineReceiptCards && (chat.receipt_cards || []).length) {
    html += renderMessage({
      role: 'assistant',
      title: 'Latest Swift receipts',
      body: 'These receipts are available from the latest action trace.',
      cards: (chat.receipt_cards || []).map(receipt => ({ type: 'receipt', receipt })),
    });
  }
...
function normalizeReceiptCard(receipt) {
  ...
  const providerRollback = output.provider_rollback || {};
  const syncStatus = swift.sync_status || output.sync_status || '';
  const rollbackVerified = providerRollback.rollback_verified ?? output.rollback_verified;
  let title = 'Swift receipt';
  if (receipt.status === 'failed' && syncStatus === 'reverted') title = 'Undo needs review';
  else if (receipt.denied_reason) title = 'Swift denied the action';
  else if (receipt.status === 'reverted' || syncStatus === 'reverted') title = 'Undo completed';
  else if (receipt.status === 'committed') title = 'Committed calendar change';
  let body = receipt.denied_reason || `Stage state: ${receipt.stage_state || swift.stage_state || 'no_op'}`;
  if (receipt.status === 'failed' && syncStatus === 'reverted') {
    body = `Provider rollback status: ${providerRollback.status || 'unverified'} · rollback verified: no`;
  } else if (receipt.status === 'reverted' || syncStatus === 'reverted') {
    body = `Sync status: ${syncStatus || 'reverted'}${rollbackVerified === undefined || rollbackVerified === null ? '' : ` · rollback verified: ${rollbackVerified ? 'yes' : 'no'}`}`;
  }
  return {
    type: 'receipt',
...
function renderReceiptCard(card) {
  ...
  const rollback = card.rollback_handle_id && card.status === 'committed' ? `<button class="secondary undo-btn" data-testid="undo-action" data-rollback="${escapeHtml(card.rollback_handle_id)}">Undo</button>` : '';
...
function renderActionQueueCard(action) {
  const body = action.status === 'denied' && action.denied_reason ? action.denied_reason : action.why_user_sees_it;
...
async function sendGoal(goal) {
  setPending(true);
  const startMessageCount = ((app.state || {}).chat || {}).messages?.length || 0;
  const request = api('/api/plans', {method: 'POST', body: {goal, commit: false}});
  request.then(state => {
    if (!app.pending) refresh(state);
    return state;
  }).catch(err => {
    if (!app.pending) showToast(err.message);
  });
  try {
    const state = await Promise.race([request, pollForAssistantTurn(startMessageCount)]);
    await refresh(state);
  } catch (err) {
