...
class DogfoodSessionState:
    ...
    def __post_init__(self) -> None:
        ...
        self.replay.set_jsonl_path(self.run_dir / "replay.jsonl")
    ...
    def _route_turn(self, goal: str):
        ...
        # structured intent. Until then, record the deterministic router as the
        # active policy component so training can see routing decisions.
        ...
        routed.evidence = dict(routed.evidence) | {"legacy_intent": legacy_intent}
        return routed
    ...
    def persist(self) -> None:
        self.run_dir.mkdir(parents=True, exist_ok=True)
        atomic_write_json(self.run_dir / "session_state.json", self._state_payload())
        atomic_write_json(self.run_dir / "latest_session.json", self.snapshot())
