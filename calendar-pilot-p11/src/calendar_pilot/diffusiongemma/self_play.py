...
class UserSimulator:
    ...
    `sim_v1` preserves the historical behavior. `sim_v2` is seed/profile
    grounded and does not read the candidate's predicted reward heads.
    """
    ...
    def respond(self, candidate: CandidateCalendarAction, observation: RawCalendarObservation | None = None, biography: UserBiography | None = None) -> SimulatedResponse:
        if self.simulator_version == "sim_v2":
    ...
    def _respond_v2(self, candidate: CandidateCalendarAction, observation: RawCalendarObservation | None, biography: UserBiography | None) -> SimulatedResponse:
        if candidate.intent == "do_nothing":
            return SimulatedResponse("ignored", 0.0, "sim_v2 baseline no-op")
        local_hour = observation.device_context.local_hour if observation is not None else 9
        fatigue = float(getattr(biography, "notification_fatigue", 0.0) if biography is not None else 0.0)
        ...
        if local_hour in bad_hours:
            p_accept -= 0.18
        p_accept -= min(0.25, fatigue * 0.35)
        ...
        if roll < p_accept:
            ...
            if self.random.random() < undo_risk:
                return SimulatedResponse("undone", -1.6, "sim_v2 undo: ground-truth reversibility/social friction exceeded tolerance")
            return SimulatedResponse("accepted", 1.0, "sim_v2 accepted from seed/profile response window and reversible value")
        if roll < p_accept + 0.16:
            return SimulatedResponse("ignored", -0.35, "sim_v2 ignored: timing/fatigue made action non-salient")
        return SimulatedResponse("rejected", -0.9, "sim_v2 rejected by seed/profile action-family boundary")
