...
class ReplayBuffer:
    ...
    def append_generic(self, record_type: str, payload: dict[str, Any], *, record_id: str | None = None, trace_id: str | None = None, causal_parent_id: str | None = None, record_schema_version: str = REPLAY_SCHEMA_VERSION) -> str:
        ...
        self.append_record(ReplayRecord(record_type=record_type, record_id=rid, trace_id=trace, causal_parent_id=causal_parent_id, payload=payload, record_schema_version=record_schema_version))
        return rid
    ...
    def append_frontier_generation(
        ...
        *,
        ...
        return self.append_generic("frontier_generation", payload, record_id=rid, trace_id=trace_id, causal_parent_id=causal_parent_id)
    ...
    def append_provider_transaction(self, *, operation: str, transaction: dict[str, Any], trace_id: str, causal_parent_id: str | None = None) -> str:
        ...
        return self.append_generic("provider_transaction", payload, record_id=f"provider_transaction:{provider_id}:{operation}:{digest}", trace_id=trace_id, causal_parent_id=causal_parent_id)

    def append_tuning_reduction(self, payload: dict[str, Any], *, trace_id: str = "tuning_reduction", causal_parent_id: str | None = None) -> str:
        digest = hashlib.sha1(json.dumps(payload, sort_keys=True, default=str).encode("utf-8")).hexdigest()[:12]
        return self.append_generic("tuning_reduction", payload, record_id=f"tuning_reduction:{digest}", trace_id=trace_id, causal_parent_id=causal_parent_id)
    ...
    def append_artifact_ref(self, *, artifact_type: str, path: str | Path, trace_id: str, causal_parent_id: str | None = None, extra: dict[str, Any] | None = None) -> str:
        ...
        digest = hashlib.sha1(json.dumps(payload, sort_keys=True, default=str).encode("utf-8")).hexdigest()[:12]
        return self.append_generic("artifact_ref", payload, record_id=f"artifact_ref:{artifact_type}:{digest}", trace_id=trace_id, causal_parent_id=causal_parent_id)
