...
**Status input (from the completed `P12-test.md` run `20260703T224814Z-p12`, updated by Stage D2 evidence):** overall decision **pass**. All deterministic, fixture, contract, invariant, browser, and release gates green (`make p12-release` → `ok: true`). The former live EventKit permission/environment hold is resolved on this machine: Stage D2 restored sandbox-targeted EventKit probing, created/configured `CalendarPilot SelfPlay`, completed provider-backed `swift_ipc_eventkit_sandbox` self-play for `create_prep_block`, verified provider readback, and rolled back cleanly. Remaining explicit holds are evidence-only: calibration (`matched_examples: 0` — cold start), `create_prep_block` autonomy promotion (insufficient human feedback + sim-real calibration volume), and the documented placebo/thin gate follow-ups. Two data-quality flags recorded in the fixture measurement snapshot must still be carried forward, not forgotten: `OTHER_intent_rate: 0.1429` (above the 0.10 gate at fixture scale) and `expected_intent_hit_rate: 0.0`.
