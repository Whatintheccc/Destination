...
| 2026-07-01 | P7 | Planck | No-go for P8: live provider writes could be planned from fixture observations and authorized by `SwiftKernelStub`; EventKit commit/rollback was not transactional/idempotent enough; permission POST returned the wrong frontend shape; EventKit conflict truth failed open; rollback snapshots could expose private calendar details; bridge timeouts could break health/state; release evidence did not include a packaged live EventKit probe. | Review fixes are implemented in the follow-up P7 commit and await re-review before P8 starts. |
