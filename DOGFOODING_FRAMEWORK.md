...
| 2026-07-01 | P7 | Planck | No-go for P8: live provider writes could be planned from fixture observations and authorized by `SwiftKernelStub`; EventKit commit/rollback was not transactional/idempotent enough; permission POST returned the wrong frontend shape; EventKit conflict truth failed open; rollback snapshots could expose private calendar details; bridge timeouts could break health/state; release evidence did not include a packaged live EventKit probe. Second review found two remaining restart-boundary issues: `live_provider` restores did not rehydrate Swift IPC undo handles, and stale fixture candidates could be restored after an EventKit observation became active. | Both rounds of review fixes are implemented and await final P7 re-review before P8 starts. |
