...
class NvidiaNIMPolicyClient:
    ...
    def generate_candidate_frontier(
        ...
        *,
        ...
        payload = {
            "model": self.model,
            "messages": [{"role": "user", "content": self._frontier_prompt(goal, observation, biography, limit=limit)}],
            "max_tokens": 2200,
            "temperature": 0.2,
            "top_p": 0.9,
            "stream": False,
        ...
        parsed = self._parse_frontier_payload(self._extract_text(data), observation, limit=limit)
    ...
    def _parse_frontier_payload(text: str, observation: RawCalendarObservation, *, limit: int) -> dict[str, Any]:
        ...
        for idx, item in enumerate(raw_candidates[:max(1, limit)]):
            if not isinstance(item, dict):
                raise LiveDiffusionGemmaSchemaError(f"generated candidate {idx} is not an object")
            try:
                candidate = CandidateCalendarAction.from_dict(item)
            except Exception as exc:
                raise LiveDiffusionGemmaSchemaError(f"generated candidate {idx} failed CandidateCalendarAction validation: {exc}") from exc
            ...
            if not candidate.actions:
                raise LiveDiffusionGemmaSchemaError(f"generated candidate {candidate.candidate_id} has no actions")
        ...
        return {"candidates": candidates, "policy_summary": str(payload.get("policy_summary") or ""), "validation_errors": validation_errors}

    def _frontier_prompt(self, goal: str, observation: RawCalendarObservation, biography: UserBiography, *, limit: int) -> str:
        return json.dumps({
